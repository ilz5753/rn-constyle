...
export type StyleKey = keyof typeof CommonStyles | false | undefined;
/**
...
export const getStyle = (keys?: StyleKey | StyleKey[]) => {
  ...
  let exe = (key: StyleKey) => {
    if (key && key.length !== 0) o = { ...o, ...CommonStyles[key] };
  };
  if (keys) {
    const keyArray = (Array.isArray(keys) ? keys : [keys]) as StyleKey[];
...
export const row = (isRTL = false, rGap = 0, cGap = 0) => [
